...
def lineTouchCircle(startX, startY, endX, endY, circleX, circleY, radius):
    ...
    # unit vec
    u = ((circleX - startX) * x_prime + (circleY - startY) * y_prime) / float(squaredDist)
...
def doesArmTouchObjects(armPosDist, objects, isGoal=False):
    ...
    for arms in armPosDist:
        ...
        for obj in objects:
            ...
            circleY = obj[1]
            radius = obj[2]
            if isGoal and lineTouchCircle(startX, startY, endX, endY, circleX, circleY, radius):
                return True
            if (not isGoal and lineTouchCircle(startX, startY, endX, endY, circleX, circleY, radius + padding)):
                return True
    return False
