...
def viterbi_2(train, test):
    ...
    freq_p_hapax_tag = {}
    for word_tag_pair in word_tag_freq:
        tag = word_tag_pair[1]
        if word_tag_freq[word_tag_pair] == 1:
            hapax_word_count += 1
            if tag in freq_p_hapax_tag:
                freq_p_hapax_tag[tag] += 1
            else:
                freq_p_hapax_tag[tag] = 1
            ...
            for tag_idx in range(cols):
                ...
                if word_idx == 0:
                    if (word,tag) not in emission_prob:
                        if (tag not in freq_p_hapax_tag or freq_p_hapax_tag[tag] == 0):
                            freq_p_hapax_tag[tag] = 0.00001
                        temp_alpha = alpha * freq_p_hapax_tag[tag] / hapax_word_count
                        prob = temp_alpha / (tag_freq[tag] + temp_alpha * (unique_tag_per_word[tag] + 1))
                    ...
                    for prev_tag in tags:
                        if (word, tag) not in emission_prob:
                            if (tag not in freq_p_hapax_tag or freq_p_hapax_tag[tag] == 0):
                                freq_p_hapax_tag[tag] = 0.00001
                            temp_alpha = alpha * freq_p_hapax_tag[tag] / hapax_word_count
                            prob = (0 + temp_alpha) / (tag_freq[tag] + temp_alpha * (unique_tag_per_word[tag] + 1))
                            emission_prob[(word,tag)] = -1000
                            emission_prob[(word,tag)] = math.log(prob)
                        if (prev_tag, tag) not in transition_prob:
                            transition_prob[(prev_tag, tag)] = -1000000000
